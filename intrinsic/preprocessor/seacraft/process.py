...
def data_prep(path):
        ...
        # Dropping the text columns which have identifiers
        cols = training_x.select_dtypes(include='object').columns
        print('cols = ' + str(cols))
        print('no. of cols = ' + str(len(cols)))
        if (len(cols) != 0):
            training_x = training_x.drop(cols, axis=1)
        if ('version' in training_x.columns):
            training_x = training_x.drop('version', axis=1)
...
for i in range(len(files)):
    ...
    print(training_data)
    training_data.to_csv('processed_data/' + project + '/processed_' + name + '.csv', index=False)
